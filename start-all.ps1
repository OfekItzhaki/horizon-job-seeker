...
# Start Backend
Write-Host "[1] Starting Backend Server (Port 3001)..." -ForegroundColor Green
Start-Service -Name "Backend Server" -Path "$PSScriptRoot\backend" -Command "npm run dev"
Start-Sleep -Seconds 3

# Start Frontend
Write-Host "[2] Starting Frontend Server (Port 3000)..." -ForegroundColor Green
...
if ($startWorker -eq "y" -or $startWorker -eq "Y") {
    Write-Host "[3] Starting Background Worker..." -ForegroundColor Green
